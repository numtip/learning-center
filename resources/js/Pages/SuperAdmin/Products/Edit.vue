...
const form = ref({
  id: props.product.id, name: props.product.name, price: props.product.price, unit: props.product.unit,
  stock: props.product.stock, category: props.product.category, description: props.product.description,
  weight: props.product.weight || '', image: null
})

const preview = ref(props.product.image_preview || (props.product.image ? `data:image/jpeg;base64,${props.product.image}` : null))
...
const submit = () => {
  ...
  Object.keys(form.value).forEach(k => {
    if (form.value[k] !== null && form.value[k] !== undefined && form.value[k] !== '') data.append(k, form.value[k])
  })

  router.post(`/admin/products/${form.value.id}`, data, {
    forceFormData: true,
    onSuccess: () => alert('✅ บันทึกการอัปเดตสำเร็จ')
  })
}
</script>
    ...
    <div class="high-contrast-layout">
      ...
      <div class="sticky-header slide-down">
        <div class="header-content">
          ...
          <h2>✏️ อัปเดตสินค้า</h2>
        </div>
        <button class="btn-glow warning" @click="submit" :disabled="isProcessing">
          <span v-if="!isProcessing">💾 บันทึกข้อมูล</span>
            ...
            <div class="card-body">
              ...
              >
                ...
                <div v-if="preview" class="preview-box">
                  ...
                  <label for="img_upload" class="overlay-btn">อัปโหลดรูปใหม่แทนที่</label>
                </div>
                ...
                <label v-else for="img_upload" class="upload-prompt">
                  ...
                  <p>เพื่ออัปเดตหน้าปกใหม่</p>
                </label>
              </div>
              <p class="img-hint" v-if="preview">ℹ️ หากไม่ต้องการเปลี่ยนรูป ไม่ต้องอัปโหลดใหม่</p>
            </div>
          </div>
        </div>
      </div>
...
/* Sticky Header - แก้ไขจุดที่เขียน CSS ผิดแล้วตรงนี้ครับ */
...
.sticky-header h2 { margin: 0; font-size: 26px; font-weight: 800; background: linear-gradient(135deg, #f59e0b, #ea580c); -webkit-background-clip: text; -webkit-text-fill-color: transparent; }

/* Vibrant Glow Button Warning */
.btn-glow.warning { background: linear-gradient(45deg, #f59e0b, #e67e22); box-shadow: 0 12px 24px rgba(230, 126, 34, 0.45); border: none; color: white; padding: 14px 32px; border-radius: 16px; font-size: 17px; font-weight: 700; cursor: pointer; transition: all 0.3s; font-family: inherit; border: 1px solid rgba(255,255,255,0.3); }
.btn-glow.warning:hover:not(:disabled) { transform: translateY(-4px); box-shadow: 0 18px 30px rgba(230, 126, 34, 0.55); }
...
.input-float input:focus, .input-float select:focus, .input-float textarea:focus { background: #ffffff; border-color: #f59e0b; box-shadow: 0 10px 20px rgba(245,158,11,0.08); }
...
.input-float textarea:focus ~ label, .input-float textarea:not(:placeholder-shown) ~ label { top: 7px; font-size: 12px; color: #d35400; font-weight: 700; }
.req { color: #dc2626; }
...
.dropzone:hover, .drag-over { border-color: #f59e0b; background: #fffbeb; }
...
.upload-prompt h4 { margin: 0 0 6px; color: #f59e0b; font-size: 17px; font-weight: 700; }
...
.img-hint { font-size: 13px; color: #6b7280; text-align: center; margin: 0; font-weight: 500; }
