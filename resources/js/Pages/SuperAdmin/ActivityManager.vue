...
// ฟังก์ชันบีบอัดรูปภาพ (ลดขนาดไฟล์ก่อนอัปโหลด)
...
const submitForm = async () => {
  ...
  const url = isEdit.value ? `/superadmin/activity-news/${form.value.id}` : '/superadmin/activity-news'
...
const remove = (id) => {
  if (confirm('คุณแน่ใจหรือไม่ที่จะลบข่าวกิจกรรมนี้?')) {
    router.delete(`/superadmin/activity-news/${id}`)
  }
}
  ...
  <AdminLayout>
    <div class="admin-wrapper">
      <div class="header-section">
        <div>
          <h2 class="page-title">🎉 จัดการข่าวกิจกรรม</h2>
          <p class="page-subtitle">เพิ่ม ลบ และแก้ไข ข่าวกิจกรรมต่างๆ ของศูนย์</p>
        </div>
        <button class="btn-primary" @click="openAdd">
          <span class="icon">➕</span> สร้างข่าวใหม่
        </button>
      </div>
      ...
      <div class="content-grid">
        <div v-for="n in news" :key="n.id" class="elegant-card">
          <div class="card-image-wrapper">
            ...
            <div class="badge-overlay">ข่าวกิจกรรม</div>
          </div>
      ...
      <div v-if="showModal" class="modal-overlay" @click.self="showModal = false">
        <div class="modal-container">
          <div class="modal-header">
            <h3>{{ isEdit ? '✏️ แก้ไขข่าวกิจกรรม' : '➕ สร้างข่าวกิจกรรมใหม่' }}</h3>
            <button class="btn-close" @click="showModal = false">✕</button>
          </div>

          <div class="modal-body">
            <div class="form-group">
              <label>หัวข้อข่าว <span class="text-red">*</span></label>
              <input v-model="form.title" class="modern-input" placeholder="ระบุหัวข้อข่าวกิจกรรม" />
            </div>
            <div class="form-group">
              <label>รายละเอียด <span class="text-red">*</span></label>
              <textarea v-model="form.description" class="modern-input textarea" placeholder="เนื้อหาและรายละเอียดของกิจกรรม..."></textarea>
            </div>

            <div class="upload-section">
              <div class="upload-cover">
                <label class="upload-label">🖼️ รูปภาพหน้าปก (Cover Image) <small>แนะนำภาพแนวนอน</small></label>
                <input type="file" class="modern-file" @change="e => onFile(e, 'image1')" accept="image/*" />
              </div>

              <div class="upload-gallery">
                <label class="upload-label">📸 รูปภาพประกอบรายละเอียด (Gallery - สูงสุด 4 รูป)</label>
              ...
              <p class="upload-note">* ระบบจะทำการบีบอัดภาพอัตโนมัติเพื่อลดความล่าช้าในการโหลด</p>
            </div>
          </div>
...
.page-title { font-size: 1.75rem; font-weight: 700; color: #064e3b; margin: 0; }
.page-subtitle { color: #6b7280; font-size: 0.9rem; margin-top: 0.25rem; }

/* Buttons */
.btn-primary { background: #10b981; color: white; border: none; padding: 0.75rem 1.5rem; border-radius: 12px; font-weight: 600; cursor: pointer; transition: 0.3s; box-shadow: 0 4px 6px -1px rgba(16, 185, 129, 0.2); }
...
.elegant-card:hover { transform: translateY(-5px); box-shadow: 0 20px 25px -5px rgba(0, 0, 0, 0.1); border-color: #d1fae5; }
...
.badge-overlay { position: absolute; top: 1rem; left: 1rem; background: rgba(255, 255, 255, 0.9); color: #064e3b; padding: 0.25rem 0.75rem; border-radius: 20px; font-size: 0.75rem; font-weight: 600; backdrop-filter: blur(4px); }
...
.modern-input:focus { outline: none; border-color: #10b981; box-shadow: 0 0 0 3px rgba(16, 185, 129, 0.1); }
...
.upload-note { font-size: 0.75rem; color: #10b981; margin: 1rem 0 0 0; text-align: center; }
...
.btn-submit { background: #10b981; border: none; color: white; padding: 0.6rem 1.5rem; border-radius: 10px; font-weight: 600; cursor: pointer; transition: 0.2s; }
