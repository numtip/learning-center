...
const resetForm = () => {
  form.value = { id: null, title: '', description: '', image1: null, image2: null, image3: null, image4: null, image5: null }
}
...
const submitForm = async () => {
  ...
  const url = isEdit.value ? `/superadmin/articles-manager/${form.value.id}` : '/superadmin/articles-manager'
...
const remove = (id) => {
  if (confirm('ยืนยันที่จะลบบทความนี้ใช่หรือไม่?')) {
    router.delete(`/superadmin/articles-manager/${id}`)
  }
}
    ...
    <div class="admin-wrapper">
      <div class="header-section">
        <div>
          ...
          <p class="page-subtitle">รวบรวมเนื้อหาสาระและความรู้ต่างๆ สำหรับเผยแพร่</p>
        </div>
        <button class="btn-primary" @click="openAdd">
          <span class="icon">➕</span> เขียนบทความใหม่
        </button>
      </div>
      ...
      <div class="content-grid">
        <div v-for="n in news" :key="n.id" class="elegant-card">
          <div class="card-image-wrapper">
            ...
            <div class="badge-overlay">บทความ</div>
          </div>
      ...
      <div v-if="showModal" class="modal-overlay" @click.self="showModal = false">
        <div class="modal-container">
          <div class="modal-header">
            <h3>{{ isEdit ? '✏️ แก้ไขบทความ' : '➕ สร้างบทความใหม่' }}</h3>
            <button class="btn-close" @click="showModal = false">✕</button>
          </div>

          <div class="modal-body">
            <div class="form-group">
              <label>ชื่อบทความ <span class="text-red">*</span></label>
              <input v-model="form.title" class="modern-input" placeholder="ระบุหัวข้อบทความที่น่าสนใจ" />
            </div>
            <div class="form-group">
              <label>เนื้อหาบทความ <span class="text-red">*</span></label>
              <textarea v-model="form.description" class="modern-input textarea" placeholder="บรรยายเนื้อหาบทความ..."></textarea>
            </div>

            <div class="upload-section">
              <div class="upload-cover">
                <label class="upload-label">🖼️ รูปภาพหน้าปก (Cover Image) <small>แสดงเป็นภาพหลัก</small></label>
                <input type="file" class="modern-file" @change="e => onFile(e, 'image1')" accept="image/*" />
              </div>

              <div class="upload-gallery">
                <label class="upload-label">📸 รูปภาพประกอบเนื้อหา (Gallery - สูงสุด 4 รูป)</label>
                <div class="gallery-grid">
                  <input type="file" class="modern-file small" @change="e => onFile(e, 'image2')" accept="image/*" title="รูปประกอบ 1" />
                  <input type="file" class="modern-file small" @change="e => onFile(e, 'image3')" accept="image/*" title="รูปประกอบ 2" />
                  <input type="file" class="modern-file small" @change="e => onFile(e, 'image4')" accept="image/*" title="รูปประกอบ 3" />
                  <input type="file" class="modern-file small" @change="e => onFile(e, 'image5')" accept="image/*" title="รูปประกอบ 4" />
                </div>
              </div>
              <p class="upload-note">* ขนาดรูปจะถูกจัดการอัตโนมัติเพื่อให้เว็บทำงานได้ลื่นไหล</p>
            </div>
          </div>
          ...
          <div class="modal-footer">
            ...
            <button class="btn-submit" @click="submitForm" :disabled="isProcessing">
              {{ isProcessing ? '⏳ กำลังเตรียมรูปภาพ...' : '💾 บันทึกบทความ' }}
            </button>
          </div>
        </div>
      </div>
...
/* ใช้ Style แบบเดียวกันเป๊ะๆ เพื่อให้หน้าเว็บดูกลมกลืนกันทั้งระบบ */
...
.page-title { font-size: 1.75rem; font-weight: 700; color: #0284c7; margin: 0; } /* เปลี่ยนสีหัวข้อให้ต่างกันนิดหน่อย */
.page-subtitle { color: #6b7280; font-size: 0.9rem; margin-top: 0.25rem; }

.btn-primary { background: #0284c7; color: white; border: none; padding: 0.75rem 1.5rem; border-radius: 12px; font-weight: 600; cursor: pointer; transition: 0.3s; box-shadow: 0 4px 6px -1px rgba(2, 132, 199, 0.2); }
...
.elegant-card:hover { transform: translateY(-5px); box-shadow: 0 20px 25px -5px rgba(0, 0, 0, 0.1); border-color: #bae6fd; }
...
.badge-overlay { position: absolute; top: 1rem; left: 1rem; background: rgba(255, 255, 255, 0.9); color: #0284c7; padding: 0.25rem 0.75rem; border-radius: 20px; font-size: 0.75rem; font-weight: 600; backdrop-filter: blur(4px); }
...
.card-desc { font-size: 0.875rem; color: #4b5563; line-height: 1.5; display: -webkit-box; -webkit-line-clamp: 3; -webkit-box-orient: vertical; overflow: hidden; margin-bottom: 1rem; flex: 1; }
...
.modern-input:focus { outline: none; border-color: #0284c7; box-shadow: 0 0 0 3px rgba(2, 132, 199, 0.1); }
...
.upload-note { font-size: 0.75rem; color: #0284c7; margin: 1rem 0 0 0; text-align: center; }
...
.btn-submit { background: #0284c7; border: none; color: white; padding: 0.6rem 1.5rem; border-radius: 10px; font-weight: 600; cursor: pointer; transition: 0.2s; }
