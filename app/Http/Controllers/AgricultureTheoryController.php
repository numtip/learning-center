...
class AgricultureTheoryController extends Controller
{
    public function index()
    {
        $detailRaw = DB::table('mju_agriculture_theory_detail')->first();

        $activities = DB::table('mju_agriculture_theory_activities')
        ...
        $pins = DB::table('mju_map_mark as m')
            ->join('mju_agriculture_theory_activities as a', 'm.activity_id', '=', 'a.id')
            ->where('m.center_id', 1)
            ->select(
                'm.id',
                'm.center_id',
        ...
        return Inertia::render('Center/Royal', [
            'detail'     => $detail,
