...
class FarmController extends Controller
{
    ...
    {
        $detailRaw = DB::table('mju_farm_demo_detail')->first();

        $activities = DB::table('mju_farm_demo_activities')
        ...
        $pins = DB::table('mju_map_mark as m')
            ->join('mju_farm_demo_activities as a', 'm.activity_id', '=', 'a.id')
            ->where('m.center_id', 2)
            ->select(
                'm.id',
                'm.center_id',
        ...
        return Inertia::render('Center/Farm', [
            'detail'     => $detail,
