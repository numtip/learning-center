...
class LannaController extends Controller
{
    public function index()
    {
        $detailRaw = DB::table('mju_culture_agriculture_center_detail')->first();

        $activities = DB::table('mju_culture_agriculture_center_activities')
        ...
        $pins = DB::table('mju_map_mark as m')
            ->join('mju_culture_agriculture_center_activities as a', 'm.activity_id', '=', 'a.id')
            ->where('m.center_id', 3)
            ->select(
                'm.id',
                'm.center_id',
        ...
        return Inertia::render('Center/Lanna', [
            'detail'     => $detail,
